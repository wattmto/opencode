...
- `auto_commit`: Whether OpenCode should automatically commit and push dirty working tree changes after it runs. Defaults to **true**. Set this to **false** for comment-only or analysis-only workflows, or when earlier workflow steps may leave unrelated changes in the working tree.
